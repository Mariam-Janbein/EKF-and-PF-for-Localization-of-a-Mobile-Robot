...
class Field:
    ...
    def G(self, state_vec, control_vec):
        """Compute the Jacobian of the dynamics with respect to the state."""
        prev_x, prev_y, prev_theta = state_vec.ravel()
        rot1, trans, rot2 = control_vec.ravel()
        _, _, theta = state_vec.ravel()
        rot1, trans, _ = control_vec.ravel()
        theta1 = theta + rot1
    ...
    def V(self, state_vec, control_vec):
    ...
    def H(self, state_vec, landmark_idx):
        """Compute the Jacobian of the observation with respect to the state."""
        prev_x, prev_y, prev_theta = state_vec.ravel()
        x_r, y_r, theta = state_vec.ravel()
        x_l = self.MARKER_X_POS[landmark_idx]
        y_l = self.MARKER_Y_POS[landmark_idx]
